...
from mxnet import initializer as mxinit
...
from  pictureslib import CluodStorage;
import yaml;
import math;
...
def parse_args():
    ...
    parser.add_argument('--train-data-yml',type=str,default='trainfiles.yml',
                        help='dataset input file')
    parser.add_argument('--force-period',type=int,default=-1,
                        help='for training and validation use only one period')

    parser.add_argument('--force-class',type=str,
                        help="force class for clasification if present, one of: buildings, forest, water (default: none)")
    parser.add_argument('--randomize-weights',action='store_true',
                        help="force weights randomization before calculations (using normal distribution sigma=0.5")


    # the parser
    args = parser.parse_args()

    _classes = ['buildings','forest','water']
    if args.force_class is None:
      args.force_class = -1
    elif args.force_class in _classes:
      args.force_class = _classes.index(args.force_class) + 1
...
class Trainer(object):
    def __init__(self, args, logger):
        ...
        data = yaml.full_load(open(args.train_data_yml))
        trainset = CluodStorage.CloudSegmentationDataSetV2(data, num_shards=kv.num_workers , shard=kv.rank, force_period = args.force_period,force_class = args.force_class, **data_kwargs)
        valset = CluodStorage.CloudSegmentationDataSetV2(data,the_test_set = True,num_shards = kv.num_workers, shard=kv.rank, force_period = args.force_period, force_class = args.force_class, **data_kwargs);

        CluodStorage.CloudSegmentationDataSetV2.NUM_CLASS = num_class = 4 if args.force_class == -1 else 2
        ...
        if args.model_zoo is not None:
            ...
            model = get_segmentation_model(model=args.model, dataset=args.dataset,
                                           ...
                                           norm_kwargs=args.norm_kwargs, aux=args.aux,
                                           base_size=args.base_size, crop_size=args.crop_size, pretrained_base=True)
            #model.load_parameters(get_model_file('%s_resnet50_ade' % args.model),allow_missing=True) # deeplab_resnet50_ade
        ...
        if args.randomize_weights:
            ...
            model.collect_params().initialize(init,force_reinit=True)
        ...
        """
        criterion = FocalLoss(num_class = 4 if args.force_class == -1 else 2,sparse_label=False)
        ...
        self.optimizer = gluon.Trainer(self.net.module.collect_params(), args.optimizer,
                                       optimizer_params, kvstore=kv, update_on_kvstore = False)
        # evaluation metrics
    ...
    def training(self, epoch):
        ...
        train_loss = 0.0
        for i, (data, target, period) in enumerate(tbar):
